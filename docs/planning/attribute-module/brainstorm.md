...
- Predefined choices (`attribute_values`, `attribute_swatch_values`) and typed dynamic values (`attribute_*_values`) stored centrally in @czo/attribute
...
- [ ] `attribute_swatch_values` table for SWATCH (color + image_url)
- [ ] Typed value tables in module: `attribute_text_values`, `attribute_numeric_values`, `attribute_boolean_values`, `attribute_date_values`, `attribute_file_values`, `attribute_richtext_values`, `attribute_reference_values`
- [ ] Value reordering (position) for DROPDOWN/MULTISELECT/SWATCH
...
- `attribute_text_values` (PLAIN_TEXT, RICH_TEXT)
...
| id | UUID | Primary key |
| attribute_id | UUID | FK to attributes |
| value | (typed) | TEXT, NUMERIC, BOOLEAN, TIMESTAMP, etc. |
| created_at | TIMESTAMP | |
| updated_at | TIMESTAMP | |
...
- `TEXT` / `RICH_TEXT` → `attribute_text_values`
...
**Key decisions made:**
1. Hybrid storage: predefined choices in `attribute_values` (DROPDOWN/MULTISELECT) and `attribute_swatch_values` (SWATCH), dynamic values in typed tables in @czo/attribute
2. SWATCH has its own table `attribute_swatch_values` with `color` (hex) and `image_url` fields
3. No AttributeGroup in MVP
4. Typed value tables (`attribute_text_values`, etc.) live in @czo/attribute module, consumers create junction tables only
5. REFERENCE type uses `reference_entity` field to specify target entity type
...
13. **Consumer pattern is customizable**: Each consumer can adapt the junction table structure to their specific needs
