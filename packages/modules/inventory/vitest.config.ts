...
// Alias `@czo/inventory/*`, `@czo/stock-location/*`, and `@czo/auth/*` to local
// `src/` rather than each package's own `dist/`. Without this, vitest follows
// the package.json `exports` "default" condition into `dist/`, which causes a
// dual-class identity problem. The E2E harness boots the real auth module, so
// auth's src and all the subpaths it imports internally must resolve to one
// realm — mirrors `packages/modules/stock-location/vitest.config.ts`. Subpath
// aliases are listed before bare names so rollup's prefix matcher resolves them
// first.
export default defineConfig({
  resolve: {
    alias: {
      '@czo/inventory/services': resolve(__dirname, 'src/services/index.ts'),
      '@czo/inventory/graphql': resolve(__dirname, 'src/graphql/index.ts'),
      '@czo/inventory/schema': resolve(__dirname, 'src/database/schema.ts'),
      '@czo/inventory/relations': resolve(__dirname, 'src/database/relations.ts'),
