...
// Only `checkMembership` is exercised by ApiKey.layer — the rest of
...
function makeTestLayer(db: object) {
  ...
  return ApiKey.layer.pipe(Layer.provide(Layer.mergeAll(dbLayer, orgStub)))
}
...
describe('apiKeyServiceLive — verify / validate', () => {
  ...
  it('empty plainKey → InvalidApiKey (no DB hit)', async () => {
    ...
    const program = Effect.gen(function* () {
      const svc = yield* ApiKey.ApiKeyService
      return yield* svc.verify('')
    })
    await expectFailure(program.pipe(Effect.provide(makeTestLayer(db))), ApiKey.InvalidApiKey)
  ...
  it('disabled key → KeyDisabled', async () => {
    ...
    await expectFailure(program.pipe(Effect.provide(makeTestLayer(db))), ApiKey.KeyDisabled)
  })
  ...
  it('permission insufficient → Unauthorized', async () => {
    ...
    await expectFailure(program.pipe(Effect.provide(makeTestLayer(db))), ApiKey.Unauthorized)
  })
  ...
  it('uPDATE returns 0 rows → UsageExceeded', async () => {
    ...
    await expectFailure(program.pipe(Effect.provide(makeTestLayer(db))), ApiKey.UsageExceeded)
  })
