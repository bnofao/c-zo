  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const findMemberInOrg = (organizationId: number, userId: number) =>
        tryDb(() => db.query.members.findFirst({ where: { organizationId, userId } }))
      ...
      return OrganizationService.of({
        ...
        checkSlug: slug =>
          tryDb(() => db.query.organizations.findFirst({ where: { slug } })).pipe(
            Effect.map(row => !row),
          ),
        ...
        create: (input, scope) =>
          Effect.gen(function* () {
            ...
            const user = yield* tryDb(() => db.query.users.findFirst({ where: { id: userId } }))
            if (!user)
              ...
              const reached = typeof scope.limit === 'function'
                ...
                : yield* tryDb(async () => {
                  const [row] = await db.select({ count: count() })
                    ...
                    .limit(1)
                  return (row?.count ?? 0) >= (scope.limit as number)
                })
              if (reached)
                return yield* Effect.fail(new OrganizationLimitReached())
            }

            const slugTaken = yield* tryDb(() =>
            ...
            const result = yield* tryDb(() => db.transaction(async (tx) => {
              const now = new Date()
        ...
        addMember: (input, memberLimit) =>
          Effect.gen(function* () {
            const user = yield* tryDb(() => db.query.users.findFirst({ where: { id: input.userId } }))
            if (!user)
              return yield* Effect.fail(new OrgUserNotFound())

            const existingMember = yield* tryDb(() =>
            ...
            const [member] = yield* tryDb(() => db.insert(members).values({
              ...input,
              role: validRole,
              createdAt: new Date(),
            }).returning())
        ...
        removeMember: (input, scope) =>
          Effect.gen(function* () {
            ...
            if (memberRoles.includes(creatorRole)) {
              const ownerCount = yield* tryDb(() => db.$count(members, and(
