...
export async function bootChannelApp(): Promise<ChannelHarness> {
  ...
  const gql: ChannelHarness['gql'] = async (query, variables = {}, token, ip) => {
  ...
  const signUp: ChannelHarness['signUp'] = async (email, name, password) => {
  ...
  const createOrganization: ChannelHarness['createOrganization'] = async (token, name, slug, ip) => {
  ...
  const setMemberRole: ChannelHarness['setMemberRole'] = (orgNumericId, userId, role) =>
